...
let exactScrollY = 0; // Tambahan untuk memori posisi presisi
const SCROLL_SPEED = 45; // Diturunkan sedikit agar pergerakan jauh lebih tenang, mulus, dan sinematik
...
function toggleAutoScroll(event) {
    ...
    if (isAutoScrolling) {
        ...
        exactScrollY = window.scrollY; // Rekam posisi persis saat tombol play ditekan
    ...
    function smoothStep(currentTime) {
        ...
        // Algoritma delta-time: Scroll berdasarkan refresh rate layar untuk animasi super mulus & anti-jitter
        const deltaTime = currentTime - lastScrollTime;
        lastScrollTime = currentTime;

        // Resync posisi diperlebar ke 15px.
        // (Toleransi 2px sebelumnya terlalu sempit. Browser HP sering terlambat 1-2 frame untuk mengupdate window.scrollY, 
        // sehingga script mengira layar salah posisi lalu menariknya mundur berkali-kali. Ini yang memicu efek "Gempa").
        if (Math.abs(window.scrollY - exactScrollY) > 15) {
            exactScrollY = window.scrollY;
        }

        if (deltaTime > 0 && deltaTime < 100) {
            const scrollAmount = (SCROLL_SPEED * deltaTime) / 1000;
            exactScrollY += scrollAmount; // Akumulasi nilai desimal secara utuh
            window.scrollTo({ top: exactScrollY, behavior: 'auto' }); // Setel absolut secara instan tanpa terhalang animasi CSS bawaan browser
        }
        
        // Memastikan deteksi ujung bawah halaman lebih toleran
...
// Menambahkan flag { passive: true } untuk membuka potensi frame-rate (FPS) maksimal saat discroll manual oleh tamu
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('scroll', () => {
        // Throttling via requestAnimationFrame untuk mencegah lag/patah-patah saat user menggulir layar dengan cepat
        if (!isScrolling) {
    ...
    const observer = new IntersectionObserver((entries) => {
        ...
        // PERBAIKAN TOTAL: Threshold 0 & Margin Negatif (-10%)
        // Memaksa layar membaca elemen keluar walau baru sebagian menyentuh batas layar (-10%).
        // Dijamin tidak ada lagi keragu-raguan/stuttering karena CSS tidak bergerak turun.
        threshold: 0, 
...
function startCountdown() {
    ...
    // Caching DOM (Mencegah mesin browser mencari elemen setiap detik secara berulang-ulang yang membuat panas HP)
    ...
    const timer = setInterval(function() {
        ...
        if (elHari) elHari.innerText = String(d).padStart(2, '0');
        if (elJam) elJam.innerText = String(h).padStart(2, '0');
        if (elMenit) elMenit.innerText = String(m).padStart(2, '0');
        if (elDetik) elDetik.innerText = String(s).padStart(2, '0');
    }, 1000);
}
